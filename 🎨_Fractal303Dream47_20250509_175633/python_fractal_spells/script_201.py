...
# Script ID 201: Op 'clifford_attractor_y', Modifiers: b=-0.17, d=-1.68
...
if __name__ == "__main__":
    ...
    modifier_b = float(-0.17)
    modifier_d = float(-1.68)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 201, prev_val_placeholder_py, **({"modifier_b": modifier_b, "modifier_d": modifier_d}))
    ...
    print(json.dumps({
        "script_id":201, "input_value":input_value, "op_type":"clifford_attractor_y", 
