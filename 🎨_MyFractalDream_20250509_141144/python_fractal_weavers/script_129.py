...
# Script ID 129: Op 'power_clip', Modifiers: modifier=0.65
op_name = "power_clip" # Make op_name available in Python global scope
...
def perform_operation(val, current_depth, script_id, **modifiers):
    ...
    try: 
        ...
        return max(-50.0, min(50.0, math.copysign(pow(abs(val), modifier), val))) if val != 0 else 0.0
...
if __name__ == "__main__":
    ...
    output_value = perform_operation(input_value, current_depth, 129, **({"modifier": modifier}))
    ...
    next_call_id_py = 112 # This will be 'None' string or a number string
    ...
    print(json.dumps({
        "script_id":129, "input_value":input_value, "op_type":"power_clip", 
