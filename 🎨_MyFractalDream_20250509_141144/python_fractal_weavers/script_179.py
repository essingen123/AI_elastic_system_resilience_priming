...
# Script ID 179: Op 'spiral_step_y', Modifiers: angle_rate=0.12, step_size=1.2
op_name = "spiral_step_y" # Make op_name available in Python global scope
...
def perform_operation(val, current_depth, script_id, **modifiers):
    ...
    try: 
        ...
        return val + math.sin(current_depth * modifier_angle_rate + script_id * 0.1) * modifier_step_size
...
if __name__ == "__main__":
    if len(sys.argv) < 5: print(json.dumps({"error": "Insufficient args", "script_id": 179})); sys.exit(1)
    ...
    modifier_angle_rate = 0.12
    modifier_step_size = 1.2

    
    output_value = perform_operation(input_value, current_depth, 179, **({"modifier_angle_rate": modifier_angle_rate, "modifier_step_size": modifier_step_size}))
    ...
    next_call_id_py = 149 # This will be 'None' string or a number string
    ...
    print(json.dumps({
        "script_id":179, "input_value":input_value, "op_type":"spiral_step_y", 
        "modifiers_used": {"modifier_angle_rate": modifier_angle_rate, "modifier_step_size": modifier_step_size}, "output_value":output_value, 
