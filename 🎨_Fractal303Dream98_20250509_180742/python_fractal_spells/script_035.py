...
# Script ID 35: Op 'de_jong_attractor_x', Modifiers: a=-1.88, b=-1.17
...
if __name__ == "__main__":
    ...
    modifier_a = float(-1.88)
    modifier_b = float(-1.17)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 35, prev_val_placeholder_py, **({"modifier_a": modifier_a, "modifier_b": modifier_b}))
    ...
    print(json.dumps({
        "script_id":35, "input_value":input_value, "op_type":"de_jong_attractor_x", 
