...
# Script ID 118: Op 'coupled_logistic_x', Modifiers: rx=3.904, cxy=0.109
...
if __name__ == "__main__":
    ...
    modifier_rx = float(3.904)
    modifier_cxy = float(0.109)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 118, prev_val_placeholder_py, **({"modifier_rx": modifier_rx, "modifier_cxy": modifier_cxy}))
    ...
    print(json.dumps({
        "script_id":118, "input_value":input_value, "op_type":"coupled_logistic_x", 
