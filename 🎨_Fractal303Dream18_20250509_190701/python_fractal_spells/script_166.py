...
# Script ID 166: Op 'de_jong_attractor_x', Modifiers: a=-2.28, b=0.62
...
def perform_operation(val, current_depth, script_id, prev_val_placeholder, **modifiers):
    ...
    try: 
        result = math.sin(modifier_a * prev_val_placeholder) - math.cos(modifier_b * val)
        return result
...
if __name__ == "__main__":
    ...
    modifier_a = float(-2.28)
    modifier_b = float(0.62)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) 
    output_value = perform_operation(input_value, current_depth, 166, prev_val_placeholder_py, **({"modifier_a": modifier_a, "modifier_b": modifier_b}))
    ...
    print(json.dumps({
        "script_id":166, "input_value":input_value, "op_type":"de_jong_attractor_x", 
