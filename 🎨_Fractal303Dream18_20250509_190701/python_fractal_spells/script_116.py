...
# Script ID 116: Op 'lyapunov_exponent_approx', Modifiers: r=3.762
...
def perform_operation(val, current_depth, script_id, prev_val_placeholder, **modifiers):
    ...
    try: 
        result = val + math.log(abs(modifier_r - 2.0 * modifier_r * ((prev_val_placeholder % 1.0 if prev_val_placeholder is not None else 0.5) if (prev_val_placeholder % 1.0 if prev_val_placeholder is not None else 0.5) is not None else 0.5))) if abs(modifier_r - 2.0 * modifier_r * ((prev_val_placeholder % 1.0 if prev_val_placeholder is not None else 0.5) if (prev_val_placeholder % 1.0 if prev_val_placeholder is not None else 0.5) is not None else 0.5)) > 1e-9 else val
        return result
...
if __name__ == "__main__":
    ...
    if next_call_id_py_str != 'None' and current_depth < (max_allowed_depth -1) and False:
        try:
