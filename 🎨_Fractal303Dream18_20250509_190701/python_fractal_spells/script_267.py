...
# Script ID 267: Op 'de_jong_attractor_y', Modifiers: c=1.85, d=-0.31
...
def perform_operation(val, current_depth, script_id, prev_val_placeholder, **modifiers):
    ...
    try: 
        result = math.sin(modifier_c * val) - math.cos(modifier_d * prev_val_placeholder)
        return result
...
if __name__ == "__main__":
    ...
    modifier_c = float(1.85)
    modifier_d = float(-0.31)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) 
    output_value = perform_operation(input_value, current_depth, 267, prev_val_placeholder_py, **({"modifier_c": modifier_c, "modifier_d": modifier_d}))
    ...
    print(json.dumps({
        "script_id":267, "input_value":input_value, "op_type":"de_jong_attractor_y", 
