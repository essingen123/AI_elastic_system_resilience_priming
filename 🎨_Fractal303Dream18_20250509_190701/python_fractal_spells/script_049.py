...
# Script ID 49: Op 'sine_power_mix', Modifiers: freq1=1.94, pow1=3, amp1=3.5, freq2=2.56, pow2=1, amp2=0.5
...
if __name__ == "__main__":
    if len(sys.argv) < 5: print(json.dumps({"error": "Insufficient args", "script_id": 49})); sys.exit(1)
    ...
    modifier_freq1 = float(1.94)
    modifier_pow1 = float(3)
    modifier_amp1 = float(3.5)
    modifier_freq2 = float(2.56)
    modifier_pow2 = float(1)
    modifier_amp2 = float(0.5)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) 
    output_value = perform_operation(input_value, current_depth, 49, prev_val_placeholder_py, **({"modifier_freq1": modifier_freq1, "modifier_pow1": modifier_pow1, "modifier_amp1": modifier_amp1, "modifier_freq2": modifier_freq2, "modifier_pow2": modifier_pow2, "modifier_amp2": modifier_amp2}))
    ...
    print(json.dumps({
        "script_id":49, "input_value":input_value, "op_type":"sine_power_mix", 
