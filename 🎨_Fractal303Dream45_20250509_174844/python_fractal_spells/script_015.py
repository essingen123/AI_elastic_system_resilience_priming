...
# Script ID 15: Op 'de_jong_attractor_x', Modifiers: a=-2.97, b=-1.76
...
if __name__ == "__main__":
    ...
    modifier_a = float(-2.97)
    modifier_b = float(-1.76)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 15, prev_val_placeholder_py, **({"modifier_a": modifier_a, "modifier_b": modifier_b}))
    ...
    print(json.dumps({
        "script_id":15, "input_value":input_value, "op_type":"de_jong_attractor_x", 
