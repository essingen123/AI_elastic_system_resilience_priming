...
# Script ID 92: Op 'modulo_chaos', Modifiers: factor=1.5, add=-5, mod=18
...
if __name__ == "__main__":
    if len(sys.argv) < 5: print(json.dumps({"error": "Insufficient args", "script_id": 92})); sys.exit(1)
    ...
    output_value = perform_operation(input_value, current_depth, 92, **({"modifier_factor": modifier_factor, "modifier_add": modifier_add, "modifier_mod": modifier_mod}))
    ...
    next_call_id = 120 if current_depth < max_allowed_depth and 1 else None
    ...
    print(json.dumps({
        "script_id":92, "input_value":input_value, "op_type":"modulo_chaos", 
