...
# Script ID 126: Op 'clifford_attractor_x', Modifiers: a=0.76, c=1.77
op_name = "clifford_attractor_x" # Make op_name global in this Python script's context
...
def perform_operation(val, current_depth, script_id, prev_val_placeholder, **modifiers):
    ...
    try:
        result = math.sin(modifiers.get("modifier_a", 0.0) * prev_val_placeholder) + modifiers.get("modifier_c", 0.0) * math.cos(modifiers.get("modifier_a", 0.0) * val)
        return result
...
if __name__ == "__main__":
    ...
    a = float(0.76) # PHP generated
    c = float(1.77) # PHP generated
    ...
    python_modifiers_dict = {"a": a, "c": c}
    
    output_value = perform_operation(input_value, current_depth, 126, prev_val_placeholder_py, **python_modifiers_dict)
    ...
    print(json.dumps({
        "script_id":126, "input_value":input_value, "op_type":op_name, # Use Python op_name
