...
# Script ID 145: Op 'coupled_logistic_x', Modifiers: rx=3.837, cxy=0.05
...
if __name__ == "__main__":
    ...
    modifier_rx = float(3.837)
    modifier_cxy = float(0.05)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 145, prev_val_placeholder_py, **({"modifier_rx": modifier_rx, "modifier_cxy": modifier_cxy}))
    ...
    print(json.dumps({
        "script_id":145, "input_value":input_value, "op_type":"coupled_logistic_x", 
