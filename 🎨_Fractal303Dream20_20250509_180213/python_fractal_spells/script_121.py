...
# Script ID 121: Op 'de_jong_attractor_y', Modifiers: c=-2.91, d=-0.55
...
if __name__ == "__main__":
    ...
    modifier_c = float(-2.91)
    modifier_d = float(-0.55)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 121, prev_val_placeholder_py, **({"modifier_c": modifier_c, "modifier_d": modifier_d}))
    ...
    print(json.dumps({
        "script_id":121, "input_value":input_value, "op_type":"de_jong_attractor_y", 
