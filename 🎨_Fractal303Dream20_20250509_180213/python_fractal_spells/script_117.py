...
# Script ID 117: Op 'clifford_attractor_y', Modifiers: b=-1.81, d=-0.93
...
if __name__ == "__main__":
    ...
    modifier_b = float(-1.81)
    modifier_d = float(-0.93)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 117, prev_val_placeholder_py, **({"modifier_b": modifier_b, "modifier_d": modifier_d}))
    ...
    print(json.dumps({
        "script_id":117, "input_value":input_value, "op_type":"clifford_attractor_y", 
