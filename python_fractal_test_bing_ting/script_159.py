...
def perform_operation(val, modifier):
    try:
        # This specific script (ID 159) uses: divide with modifier 0.8
        # Lambda: val / modifier if modifier != 0 else val + 0.1
        return val / modifier if modifier != 0 else val + 0.1
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print(json.dumps({"error": "Insufficient arguments", "script_id": 159}))
        sys.exit(1)
