...
# Script ID 198: Op 'coupled_logistic_x', Modifiers: rx=3.665, cxy=0.031
...
if __name__ == "__main__":
    ...
    modifier_rx = float(3.665)
    modifier_cxy = float(0.031)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 198, prev_val_placeholder_py, **({"modifier_rx": modifier_rx, "modifier_cxy": modifier_cxy}))
    ...
    print(json.dumps({
        "script_id":198, "input_value":input_value, "op_type":"coupled_logistic_x", 
