...
# Script ID 122: Op 'lyapunov_exponent_approx', Modifiers: r=3.92
...
if __name__ == "__main__":
    if len(sys.argv) < 5: print(json.dumps({"error": "Insufficient args", "script_id": 122})); sys.exit(1)
    ...
    modifier_r = float(3.92)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 122, prev_val_placeholder_py, **({"modifier_r": modifier_r}))
    ...
    print(json.dumps({
        "script_id":122, "input_value":input_value, "op_type":"lyapunov_exponent_approx", 
