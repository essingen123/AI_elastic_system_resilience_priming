...
# Script ID 224: Op 'de_jong_attractor_x', Modifiers: a=-1.96, b=-1.83
...
if __name__ == "__main__":
    ...
    modifier_a = float(-1.96)
    modifier_b = float(-1.83)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 224, prev_val_placeholder_py, **({"modifier_a": modifier_a, "modifier_b": modifier_b}))
    ...
    print(json.dumps({
        "script_id":224, "input_value":input_value, "op_type":"de_jong_attractor_x", 
