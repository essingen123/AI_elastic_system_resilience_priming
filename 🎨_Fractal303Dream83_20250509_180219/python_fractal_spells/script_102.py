...
# Script ID 102: Op 'coupled_logistic_x', Modifiers: rx=3.626, cxy=0.082
...
if __name__ == "__main__":
    ...
    modifier_rx = float(3.626)
    modifier_cxy = float(0.082)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 102, prev_val_placeholder_py, **({"modifier_rx": modifier_rx, "modifier_cxy": modifier_cxy}))
    ...
    print(json.dumps({
        "script_id":102, "input_value":input_value, "op_type":"coupled_logistic_x", 
