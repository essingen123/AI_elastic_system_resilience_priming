...
# Script ID 55: Op 'lyapunov_exponent_approx', Modifiers: r=3.591
...
if __name__ == "__main__":
    if len(sys.argv) < 5: print(json.dumps({"error": "Insufficient args", "script_id": 55})); sys.exit(1)
    ...
    r = float(3.591) # PHP generated

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) 
    python_modifiers_dict = {"r": r}
    output_value = perform_operation(input_value, current_depth, 55, prev_val_placeholder_py, **python_modifiers_dict)
    ...
    print(json.dumps({
        "script_id":55, "input_value":input_value, "op_type":op_name, 
