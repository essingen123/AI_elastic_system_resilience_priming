...
# Script ID 9: Op 'sine_power_mix', Modifiers: freq1=0.71, pow1=1, amp1=0.8, freq2=2.01, pow2=2, amp2=4.8
...
if __name__ == "__main__":
    ...
    modifier_freq1 = float(0.71)
    modifier_pow1 = float(1)
    modifier_amp1 = float(0.8)
    modifier_freq2 = float(2.01)
    modifier_pow2 = float(2)
    modifier_amp2 = float(4.8)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 9, prev_val_placeholder_py, **({"modifier_freq1": modifier_freq1, "modifier_pow1": modifier_pow1, "modifier_amp1": modifier_amp1, "modifier_freq2": modifier_freq2, "modifier_pow2": modifier_pow2, "modifier_amp2": modifier_amp2}))
    ...
    print(json.dumps({
        "script_id":9, "input_value":input_value, "op_type":"sine_power_mix", 
