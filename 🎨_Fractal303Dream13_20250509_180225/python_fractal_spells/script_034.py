...
# Script ID 34: Op 'de_jong_attractor_y', Modifiers: c=-1.82, d=-2.24
...
if __name__ == "__main__":
    ...
    modifier_c = float(-1.82)
    modifier_d = float(-2.24)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 34, prev_val_placeholder_py, **({"modifier_c": modifier_c, "modifier_d": modifier_d}))
    ...
    print(json.dumps({
        "script_id":34, "input_value":input_value, "op_type":"de_jong_attractor_y", 
