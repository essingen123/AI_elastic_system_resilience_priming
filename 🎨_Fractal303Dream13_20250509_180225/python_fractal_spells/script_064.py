...
# Script ID 64: Op 'sine_power_mix', Modifiers: freq1=0.13, pow1=3, amp1=1.6, freq2=2.82, pow2=3, amp2=0.8
...
if __name__ == "__main__":
    ...
    modifier_freq1 = float(0.13)
    modifier_pow1 = float(3)
    modifier_amp1 = float(1.6)
    modifier_freq2 = float(2.82)
    modifier_pow2 = float(3)
    modifier_amp2 = float(0.8)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 64, prev_val_placeholder_py, **({"modifier_freq1": modifier_freq1, "modifier_pow1": modifier_pow1, "modifier_amp1": modifier_amp1, "modifier_freq2": modifier_freq2, "modifier_pow2": modifier_pow2, "modifier_amp2": modifier_amp2}))
    ...
    print(json.dumps({
        "script_id":64, "input_value":input_value, "op_type":"sine_power_mix", 
