...
# Script ID 78: Op 'sine_power_mix', Modifiers: freq1=1.43, pow1=1, amp1=2.5, freq2=0.4, pow2=1, amp2=2.8
...
if __name__ == "__main__":
    ...
    modifier_freq1 = float(1.43)
    modifier_pow1 = float(1)
    modifier_amp1 = float(2.5)
    modifier_freq2 = float(0.4)
    modifier_pow2 = float(1)
    modifier_amp2 = float(2.8)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 78, prev_val_placeholder_py, **({"modifier_freq1": modifier_freq1, "modifier_pow1": modifier_pow1, "modifier_amp1": modifier_amp1, "modifier_freq2": modifier_freq2, "modifier_pow2": modifier_pow2, "modifier_amp2": modifier_amp2}))
    ...
    print(json.dumps({
        "script_id":78, "input_value":input_value, "op_type":"sine_power_mix", 
