...
# Script ID 135: Op 'de_jong_attractor_x', Modifiers: a=-1.3, b=-2.34
...
if __name__ == "__main__":
    ...
    modifier_a = float(-1.3)
    modifier_b = float(-2.34)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 135, prev_val_placeholder_py, **({"modifier_a": modifier_a, "modifier_b": modifier_b}))
    ...
    print(json.dumps({
        "script_id":135, "input_value":input_value, "op_type":"de_jong_attractor_x", 
