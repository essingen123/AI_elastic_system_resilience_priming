...
# Script ID 77: Op 'de_jong_attractor_y', Modifiers: c=-1.01, d=-0.39
...
if __name__ == "__main__":
    ...
    modifier_c = float(-1.01)
    modifier_d = float(-0.39)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 77, prev_val_placeholder_py, **({"modifier_c": modifier_c, "modifier_d": modifier_d}))
    ...
    print(json.dumps({
        "script_id":77, "input_value":input_value, "op_type":"de_jong_attractor_y", 
