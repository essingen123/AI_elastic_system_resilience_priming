...
# Script ID 177: Op 'lyapunov_exponent_approx', Modifiers: r=3.764
...
if __name__ == "__main__":
    if len(sys.argv) < 5: print(json.dumps({"error": "Insufficient args", "script_id": 177})); sys.exit(1)
    ...
    modifier_r = float(3.764)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 177, prev_val_placeholder_py, **({"modifier_r": modifier_r}))
    ...
    print(json.dumps({
        "script_id":177, "input_value":input_value, "op_type":"lyapunov_exponent_approx", 
