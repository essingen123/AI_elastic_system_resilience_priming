...
# Script ID 104: Op 'de_jong_attractor_x', Modifiers: a=-0.61, b=-1.57
...
if __name__ == "__main__":
    ...
    modifier_a = float(-0.61)
    modifier_b = float(-1.57)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 104, prev_val_placeholder_py, **({"modifier_a": modifier_a, "modifier_b": modifier_b}))
    ...
    print(json.dumps({
        "script_id":104, "input_value":input_value, "op_type":"de_jong_attractor_x", 
