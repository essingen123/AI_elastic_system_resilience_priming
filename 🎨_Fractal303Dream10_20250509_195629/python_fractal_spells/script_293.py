...
# Script ID 293: Op 'de_jong_attractor_x', Modifiers: a=-0.29, b=-2.05
op_name = "de_jong_attractor_x" 
...
def perform_operation(val, current_depth, script_id, prev_val_placeholder, **modifiers):
    ...
    try:
        result = math.sin(modifiers.get("modifier_a", 0.0) * prev_val_placeholder) - math.cos(modifiers.get("modifier_b", 0.0) * val)
        return result
...
if __name__ == "__main__":
    ...
    modifier_a = float(-0.29)
    modifier_b = float(-2.05)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) 
    output_value = perform_operation(input_value, current_depth, 293, prev_val_placeholder_py, **({"modifier_a": modifier_a, "modifier_b": modifier_b}))
    ...
    print(json.dumps({
        "script_id":293, "input_value":input_value, "op_type":"de_jong_attractor_x", 
        "modifiers_used": {"modifier_a": modifier_a, "modifier_b": modifier_b}, "output_value":output_value, 
