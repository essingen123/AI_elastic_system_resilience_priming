...
# Script ID 120: Op 'de_jong_attractor_x', Modifiers: a=-0.27, b=1.07
...
if __name__ == "__main__":
    if len(sys.argv) < 5: print(json.dumps({"error": "Insufficient args", "script_id": 120})); sys.exit(1)
    ...
    modifier_a = float(-0.27)
    modifier_b = float(1.07)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) 
    output_value = perform_operation(input_value, current_depth, 120, prev_val_placeholder_py, **({"modifier_a": modifier_a, "modifier_b": modifier_b}))
    ...
    print(json.dumps({
        "script_id":120, "input_value":input_value, "op_type":"de_jong_attractor_x", 
