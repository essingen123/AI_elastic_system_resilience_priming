...
# Script ID 159: Op 'de_jong_attractor_y', Modifiers: c=-0.75, d=2.49
op_name = "de_jong_attractor_y" 
...
def perform_operation(val, current_depth, script_id, prev_val_placeholder, **modifiers):
    ...
    try:
        result = math.sin(modifiers.get("modifier_c", 0.0) * val) - math.cos(modifiers.get("modifier_d", 0.0) * prev_val_placeholder)
        return result
...
if __name__ == "__main__":
    if len(sys.argv) < 5: print(json.dumps({"error": "Insufficient args", "script_id": 159})); sys.exit(1)
    ...
    modifier_c = float(-0.75)
    modifier_d = float(2.49)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) 
    output_value = perform_operation(input_value, current_depth, 159, prev_val_placeholder_py, **({"modifier_c": modifier_c, "modifier_d": modifier_d}))
    ...
    print(json.dumps({
        "script_id":159, "input_value":input_value, "op_type":"de_jong_attractor_y", 
        "modifiers_used": {"modifier_c": modifier_c, "modifier_d": modifier_d}, "output_value":output_value, 
