...
# Script ID 203: Op 'coupled_logistic_x', Modifiers: rx=3.998, cxy=0.033
...
if __name__ == "__main__":
    ...
    modifier_rx = float(3.998)
    modifier_cxy = float(0.033)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) 
    output_value = perform_operation(input_value, current_depth, 203, prev_val_placeholder_py, **({"modifier_rx": modifier_rx, "modifier_cxy": modifier_cxy}))
    ...
    print(json.dumps({
        "script_id":203, "input_value":input_value, "op_type":"coupled_logistic_x", 
