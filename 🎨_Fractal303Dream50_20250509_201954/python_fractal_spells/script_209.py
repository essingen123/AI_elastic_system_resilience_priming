...
# Script ID 209: Op 'lyapunov_exponent_approx', Modifiers: r=3.595
op_name = "lyapunov_exponent_approx" # Make op_name global in this Python script's context
...
def perform_operation(val, current_depth, script_id, prev_val_placeholder, **modifiers):
    ...
    try:
        result = val + math.log(abs(modifiers.get("modifier_r", 3.57) - 2.0 * modifiers.get("modifier_r", 3.57) * _internal_prev_norm)) if abs(modifiers.get("modifier_r", 3.57) - 2.0 * modifiers.get("modifier_r", 3.57) * _internal_prev_norm) > 1e-9 else val
        return result
...
if __name__ == "__main__":
    ...
    python_modifiers_dict = {"r": r}
    
    output_value = perform_operation(input_value, current_depth, 209, prev_val_placeholder_py, **python_modifiers_dict)
    ...
    print(json.dumps({
        "script_id":209, "input_value":input_value, "op_type":op_name, # Use Python op_name
