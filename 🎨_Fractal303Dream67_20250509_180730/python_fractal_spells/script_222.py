...
# Script ID 222: Op 'de_jong_attractor_y', Modifiers: c=-0.88, d=0.18
...
if __name__ == "__main__":
    if len(sys.argv) < 5: print(json.dumps({"error": "Insufficient args", "script_id": 222})); sys.exit(1)
    ...
    modifier_d = float(0.18)

    prev_val_placeholder_py = random.uniform(-0.5, 0.5) # General small random influence 
    output_value = perform_operation(input_value, current_depth, 222, prev_val_placeholder_py, **({"modifier_c": modifier_c, "modifier_d": modifier_d}))
    ...
    print(json.dumps({
        "script_id":222, "input_value":input_value, "op_type":"de_jong_attractor_y", 
